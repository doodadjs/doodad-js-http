...
(function() {
	...
	global.DD_MODULES['Doodad.Server.Http'] = {
		type: null,
		version: '0d',
								...
								httpInterfaces.PageFactory,
			{
				...
				create: doodad.OVERRIDE(function create(urlMappings, /*optional*/options) {
					...
					tools.forEach(urlMappings, function(maps, key) {
						// TODO: Dynamic URLs (like "/**/index.html"). Use a Map object with functions as keys as custom URL matching resolvers which will return a score number ?
						...
						tools.forEach(maps, function(map) {
							...
							if (!types._implements(map.page, httpMixIns.Page)) {
								throw new types.TypeError("Invalid page type.");
							};
							if (types.isString(map.path)) {
								map.path = tools.Path.parse(map.path);
							};
							if (types.isString(map.cachePath)) {
								map.cachePath = tools.Path.parse(map.cachePath);
							};
							if (types.isArray(map.verbs)) {
								tools.forEach(map.verbs, function(verb, i) {
									map.verbs[i] = verb.toLowerCase();
								});
							};
						});
					});
